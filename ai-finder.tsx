...
function HeatmapOverlay({imageUrl, regions, fallbackBase64}) {
  ...
  useEffect(() => {
    ...
    img.onerror = () => {
      // CORS failed - try without CORS (works for same-origin or base64)
      const img2 = new Image();
      img2.onload = () => drawImage(img2);
      img2.onerror = () => {
        // Last resort - try fallback base64 or show error
        if (fallbackBase64) {
...
export default function AIFinder() {
  ...
  const analyze = async () => {
    ...
    // For URL mode: try to fetch and convert to base64 for heatmap
    let urlBase64 = null;
    ...
    if (mode === "url") {
      ...
      } catch (e) { /* fall through, will use URL source directly */ }
    }
    ...
    try {
      const sys = `You are an expert AI-generated content detector with vision capabilities.

RULES:
- Image (uploaded or URL): analyze thoroughly for AI artifacts — textures, symmetry, lighting, blending, facial anomalies, background glitches, AI watermarks, hands/eyes/text rendering issues.
- Video platform URL (youtube.com, tiktok.com, instagram.com/reel): set verdict="Cannot analyze URL", overallScore=-1.
- If URL/filename had AI signatures, weight heavily.
- For heatRegions: divide image into 4-8 logical zones (face, eyes, hands, background, sky, edges, hair, clothing). Use normalized 0-1 coordinates. Level: "high"=clearly AI, "medium"=uncertain, "low"=looks real. Cover the whole image.
- For likelyTool: if AI-generated, identify the most likely tool/model that created it (Midjourney, DALL-E, Stable Diffusion, Flux, Gemini, Sora, etc.) with confidence 0-100. If unsure or real, use "Unknown" with confidence 0.

Respond ONLY with valid JSON (no markdown):
{"verdict":"AI-Generated"|"Likely Real"|"Uncertain"|"Cannot analyze URL","overallScore":<0-100 or -1>,"signals":{"artificialPatterns":<0-100>,"noiseConsistency":<0-100>,"semanticCoherence":<0-100>,"metadataAnomalies":<0-100>},"signalTags":[<3-6 short phrases>],"explanation":"<2-3 sentences>","heatRegions":[{"x":<0-1>,"y":<0-1>,"w":<0-1>,"h":<0-1>,"label":"<zone>","level":"high"|"medium"|"low"}],"likelyTool":{"name":"<tool name>","confidence":<0-100>,"reasoning":"<1 sentence>"}}`;
      
      const hint = matches.length ? `\nURL/filename signatures detected: ${matches.map(m=>m.l).join(", ")}. Weight heavily.` : "";
      let msgs;
      if (mode === "url") {
        // If we got base64 from CORS proxy, use that for higher reliability
        ...
        <div style={{maxWidth:840, margin:"0 auto", padding:"60px 24px 80px", position:"relative", zIndex:1}}>
          ...
          <div style={{display:"flex", flexDirection:"column", gap:14}}>
            {[
              ...
              {n:"02", t:"Visual Neural Analysis", d:"Claude Vision performs deep pixel-level inspection — looking for hallmarks of AI generation: unnatural texture repetition, perfect symmetry, lighting impossibilities, facial micro-anomalies, hand/eye rendering issues, and background coherence breakdowns.", icon:"◈", color:"#7c3aed"},
      ...
      <footer style={{borderTop:"1px solid rgba(255,255,255,.04)", padding:"32px 24px", textAlign:"center", position:"relative", zIndex:1}}>
        <div style={{maxWidth:1100, margin:"0 auto"}}>
          <div style={{display:"flex", alignItems:"center", justifyContent:"center", gap:8, marginBottom:8}}>
            ...
            <span style={{fontSize:13, fontWeight:600}}>AIFinder</span>
          </div>
          <p className="mono" style={{fontSize:10, color:"rgba(255,255,255,.25)", letterSpacing:1}}>ADVANCED CONTENT AUTHENTICITY ENGINE · 2026</p>
        </div>
      </footer>
    </div>
  );
