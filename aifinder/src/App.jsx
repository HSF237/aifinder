...
function CircleScore({score}) {
  ...
  const col = score>=60?"#ef4444":score>=30?"#f59e0b":"#22c55e";
  const d = (score/100)*c;
  return (
    <svg width="150" height="150" viewBox="0 0 150 150">
      ...
      <text x="75" y="105" textAnchor="middle" fill={col} fontSize="11" fontWeight="600" letterSpacing=".5">
        {score>=60?"AI GENERATED":score>=30?"UNCERTAIN":"AUTHENTIC"}
<<<<<<< HEAD
      </text>
    </svg>
  );
...
function TextCircleScore({score}) {
  const r=58, c=2*Math.PI*r;
  const col = score>=60?"#ef4444":score>=30?"#f59e0b":"#22c55e";
  const d = (score/100)*c;
  return (
    <svg width="150" height="150" viewBox="0 0 150 150">
      <defs>
        <linearGradient id="ringGradT" x1="0%" y1="0%" x2="100%" y2="100%">
          <stop offset="0%" stopColor={col}/>
          <stop offset="100%" stopColor={col} stopOpacity=".5"/>
        </linearGradient>
        <filter id="glowFT"><feGaussianBlur stdDeviation="3" result="b"/><feMerge><feMergeNode in="b"/><feMergeNode in="SourceGraphic"/></feMerge></filter>
      </defs>
      <circle cx="75" cy="75" r={r} fill="none" stroke="rgba(255,255,255,.05)" strokeWidth="10"/>
      <circle cx="75" cy="75" r={r} fill="none" stroke="url(#ringGradT)" strokeWidth="10"
        strokeDasharray={`${d} ${c}`} strokeDashoffset={c*.25} strokeLinecap="round" filter="url(#glowFT)"
        style={{transition:"stroke-dasharray 1.4s cubic-bezier(.22,1,.36,1)"}}/>
      <text x="75" y="68" textAnchor="middle" fill="#fff" fontSize="32" fontWeight="700" fontFamily="JetBrains Mono">{score}</text>
      <text x="75" y="86" textAnchor="middle" fill="rgba(255,255,255,.35)" fontSize="10" letterSpacing="1">/ 100</text>
      <text x="75" y="105" textAnchor="middle" fill={col} fontSize="11" fontWeight="600" letterSpacing=".5">
        {score>=60?"AI WRITTEN":score>=30?"UNCERTAIN":"LIKELY HUMAN"}
=======
    ...
    if (mode === "url") {
      ...
      try {
        ...
        if (url.startsWith("data:image/")) {
          ...
          // Try CORS proxies for http URLs
          const proxies = [
      ...
      } catch (e) { /* fall through to URL source */ }
    }
...
<<<<<<< HEAD
          {kw:"gemini watermark", brand:"Google Gemini"}, {kw:"imagefx", brand:"Google ImageFX"},
          {kw:"dall-e watermark", brand:"OpenAI DALL-E"}, {kw:"dall·e watermark", brand:"OpenAI DALL-E"},
          {kw:"midjourney watermark", brand:"Midjourney"}, {kw:"stable diffusion watermark", brand:"Stability AI"},
          {kw:"adobe firefly watermark", brand:"Adobe Firefly"},
          {kw:"sora watermark", brand:"OpenAI Sora"}, {kw:"flux watermark", brand:"Black Forest Labs Flux"},
          {kw:"kling watermark", brand:"Kling AI"}, {kw:"leonardo watermark", brand:"Leonardo AI"},
          {kw:"ideogram watermark", brand:"Ideogram"}, {kw:"runway watermark", brand:"RunwayML"},
          {kw:"ai watermark detected", brand:null},
          {kw:"generated by ai", brand:null}, {kw:"ai-generated watermark", brand:null},
          {kw:"watermark visible", brand:null}, {kw:"watermark present", brand:null},
=======
          ...
          {kw:"ai watermark", brand:null}, {kw:"watermark visible", brand:null}, {kw:"watermark present", brand:null},
          {kw:"generated by ai", brand:null}, {kw:"ai-generated watermark", brand:null},
>>>>>>> 2a2fcb0ae1a3e05f6cc80023f25ddb326ebfcfe2
        ];
  ...
  const reset = () => { setFile(null); setFileData(null); setPreview(null); setResult(null); setUrl(""); setError(""); setUrlMatches([]); setHeatRegions(null); setUrlImageData(null); setTextInput(""); setTextResult(null); };

  const analyzeText = async () => {
    setLoading(true); setTextResult(null); setError("");
    setLoadStep("Scanning writing patterns...");
    await new Promise(r => setTimeout(r, 400));
    setLoadStep("Analyzing vocabulary & burstiness...");
    await new Promise(r => setTimeout(r, 500));
    setLoadStep("Running AI signature detection...");
    try {
      ...
      else parsed.verdict = "Likely Human";
  ...
  const canAnalyze = mode==="upload" ? !!file : mode==="url" ? (url.trim().length > 10 || isDataUrl) : textInput.trim().length >= 50;
        ...
        <div style={{maxWidth:760, margin:"0 auto", padding:"60px 24px 80px", position:"relative", zIndex:1}}>
          ...
          {!result && !loading && (
            <div className="fadeUp delay-1">
              <div style={{display:"flex", gap:6, marginBottom:24, background:"rgba(255,255,255,.03)", border:"1px solid rgba(255,255,255,.06)", borderRadius:18, padding:6}}>
                {[["upload","Upload Image"],["url","Image URL"],["text","Analyze Text"]].map(([k,l])=>(
              ...
              {mode === "url" && (
                <div style={{marginBottom:18}}>
                  <div style={{position:"relative", marginBottom:10}}>
                    ...
                    <input className="input-field" value={url} onChange={e=>setUrl(e.target.value)} placeholder="https://example.com/image.jpg  or paste a data:image/... URL"
                  ...
                  {!liveCheck.matches.length && url.trim().length > 5 && !url.startsWith("data:") && (
                    <p style={{margin:0, fontSize:12, color:"rgba(255,255,255,.3)"}}>No signatures in URL — will perform full visual analysis.</p>
                  )}
                  {url.startsWith("data:image/") && (
                    <p style={{margin:0, fontSize:12, color:"rgba(167,139,250,.6)"}}>✓ Data URL detected — will analyze directly without fetching.</p>
                  )}
                </div>
              )}
                ...
                <div style={{marginBottom:18}}>
                  <div style={{position:"relative", marginBottom:10}}>
                    <textarea
                      ...
                      placeholder="Paste any text here — an article, essay, email, social post, product description, student paper... Minimum 50 characters."
                      rows={8}
                      style={{resize:"vertical", minHeight:160, lineHeight:1.7, paddingTop:14}}
                    />
                  </div>
                  <div style={{display:"flex", justifyContent:"space-between", alignItems:"center"}}>
                    <p style={{margin:0, fontSize:12, color:"rgba(255,255,255,.3)"}}>
                      {textInput.length < 50
                        ...
                        : `${textInput.length.toLocaleString()} characters · ready to analyze`}
                    </p>
                    {textInput.length > 0 && (
                      <button onClick={()=>setTextInput("")} style={{fontSize:11, color:"rgba(255,255,255,.4)", background:"none", border:"1px solid rgba(255,255,255,.1)", cursor:"pointer", padding:"4px 12px", borderRadius:20, fontFamily:"inherit"}}>Clear</button>
                    )}
                  </div>
                </div>
              )}
              ...
              <button className="btn-primary" onClick={mode==="text" ? analyzeText : analyze} disabled={!canAnalyze} style={{width:"100%", padding:"15px"}}>
                {mode==="text" ? "Analyze Text →" : "Analyze Content →"}
              </button>
            </div>
          )}
          ...
          {textResult && (
            <div>
              {(() => {
                const tr = textResult;
                const tCol = tr.verdict==="AI-Generated"?"#ef4444":tr.verdict==="Likely Human"?"#22c55e":"#f59e0b";
                const tBg = tr.verdict==="AI-Generated"?"rgba(239,68,68,.06)":tr.verdict==="Likely Human"?"rgba(34,197,94,.06)":"rgba(245,158,11,.06)";
                const tBorder = tr.verdict==="AI-Generated"?"rgba(239,68,68,.25)":tr.verdict==="Likely Human"?"rgba(34,197,94,.25)":"rgba(245,158,11,.25)";
                return (
                  <div>
                    <div className="fadeUp" style={{background:tBg, border:`1px solid ${tBorder}`, borderRadius:24, padding:"32px", marginBottom:14, display:"flex", alignItems:"center", gap:28, position:"relative", overflow:"hidden"}}>
                      <div style={{position:"absolute", top:0, right:0, width:200, height:200, borderRadius:"50%", background:`radial-gradient(circle,${tCol}25,transparent 70%)`, filter:"blur(40px)"}}/>
                      <TextCircleScore score={tr.overallScore}/>
                      <div style={{flex:1, position:"relative",zIndex:1}}>
                        <div className="mono" style={{fontSize:10, textTransform:"uppercase", letterSpacing:2, color:tCol, marginBottom:8, fontWeight:600, opacity:.8}}>Text Detection Result</div>
                        <div style={{fontSize:30, fontWeight:800, color:tCol, letterSpacing:"-1px", marginBottom:8, lineHeight:1.1}}>{tr.verdict}</div>
                        <div style={{fontSize:13, color:tCol, opacity:.65, marginBottom:14}}>AI probability score: <span className="mono" style={{fontWeight:600}}>{tr.overallScore}%</span></div>
                        {tr.likelyModel && tr.likelyModel.confidence > 30 && (
                          <div style={{padding:"8px 12px", borderRadius:10, background:"rgba(0,0,0,.3)", border:`1px solid ${tCol}30`, display:"inline-block"}}>
                            <div style={{fontSize:10, color:"rgba(255,255,255,.4)", marginBottom:2, letterSpacing:.5, textTransform:"uppercase"}}>Likely Model</div>
                            <div style={{fontSize:13, fontWeight:600}}>{tr.likelyModel.name} <span className="mono" style={{color:tCol, fontSize:11, marginLeft:4}}>{tr.likelyModel.confidence}%</span></div>
                          </div>
                        )}
                      </div>
                    </div>

                    <div className="glass fadeUp delay-1" style={{padding:"22px", marginBottom:12}}>
                      <p style={{fontWeight:700, fontSize:14, marginBottom:18, letterSpacing:"-.2px"}}>◆ Signal Analysis</p>
                      <SBar label="Writing patterns (AI structure)" score={tr.signals.writingPatterns} delay={.05}/>
                      <SBar label="Vocabulary flags (AI words)" score={tr.signals.vocabularyFlags} delay={.1}/>
                      <SBar label="Sentence uniformity (low burstiness)" score={tr.signals.burstiness} delay={.15}/>
                      <SBar label="Absence of personal voice" score={tr.signals.personalVoice} delay={.2}/>
                    </div>

                    {tr.signalTags?.length > 0 && (
                      <div className="glass fadeUp delay-2" style={{padding:"20px", marginBottom:12}}>
                        <p style={{fontWeight:700, fontSize:14, marginBottom:12, letterSpacing:"-.2px"}}>● Detected Signals</p>
                        <div>{tr.signalTags.map((t,i)=>(
                          <span key={i} className="tag" style={{
                            background:tr.overallScore>=60?"rgba(239,68,68,.06)":tr.overallScore>=30?"rgba(245,158,11,.06)":"rgba(34,197,94,.06)",
                            color:tr.overallScore>=60?"#fca5a5":tr.overallScore>=30?"#fcd34d":"#86efac",
                            border:`1px solid ${tr.overallScore>=60?"rgba(239,68,68,.2)":tr.overallScore>=30?"rgba(245,158,11,.2)":"rgba(34,197,94,.2)"}`
                          }}>{t}</span>
                        ))}</div>
                      </div>
                    )}
                    ...
                    <div className="glass fadeUp delay-3" style={{padding:"20px", marginBottom:12}}>
                      ...
                      <p style={{fontSize:13.5, lineHeight:1.75, color:"rgba(255,255,255,.55)", margin:0}}>{tr.explanation}</p>
                      {tr.likelyModel?.reasoning && (
                        <p style={{fontSize:12.5, lineHeight:1.7, color:"rgba(255,255,255,.35)", margin:"10px 0 0", borderTop:"1px solid rgba(255,255,255,.06)", paddingTop:10}}>{tr.likelyModel.reasoning}</p>
                      )}
                    </div>
            ...
            <div>
              {isUrlErr ? (
                ...
                <div>
                  ...
                  <button className="btn-outline" onClick={reset} style={{width:"100%", marginTop:6}}>Analyze Another Image →</button>
                </div>
              )}
            </div>
          )}
